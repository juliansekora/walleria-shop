...
function korbHinweise(korb){
  ...
  if(druck){ const s=korb.reduce((a,k)=>a+k.preis,0)-r.betrag; const fehlt=VERSANDFREI-s;
...
const kf=q("#kForm"); if(kf){
  ...
  function kasseZeigen(){
    ...
    korb.forEach((k,i)=>{ s+=k.preis; const li=document.createElement("li"); li.innerHTML=`<img data-k="${k.img}" alt="${k.titel}"><div class="t"><b>${esc(k.titel)}</b>${artEtikett(k)}<small>${esc(groesseKurz(k))}${k.name?" · "+esc(k.name):""}</small></div><b>${eur(k.preis)}</b><button type="button" class="btn line" style="padding:4px 8px;min-height:32px" data-del="${i}" aria-label="${esc(k.titel)} entfernen" title="Entfernen">✕</button>`; ul.appendChild(li); });
    bilderSetzen(ul);
    ...
    q("#kZwischen").textContent=eur(s); q("#kVersand").textContent=eur(versand); q("#kGesamt").textContent=eur(netto+versand);
  }
  kasseZeigen();
