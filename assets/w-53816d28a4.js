...
async function kpPhoton(q){const r=await fetch(`https://photon.komoot.io/api/?q=${encodeURIComponent(q)}&lang=de&limit=5&lat=51&lon=10`);const j=await r.json();
 return (j.features||[]).map(f=>{const p=f.properties;const name=[p.name||[p.street,p.housenumber].filter(Boolean).join(" "),p.postcode,p.city||p.town||p.village||p.county,p.country].filter(Boolean).join(", ");
...
function kpPosterHTML(){
 return '<div class="kp-poster"><p class="kopf"></p><p class="koord"></p><div class="mapwrap"><svg class="ring" viewBox="0 0 200 200"><defs><path id="rp" d="M100,100 m-88,0 a88,88 0 1,1 176,0 a88,88 0 1,1 -176,0"/></defs><text><textPath href="#rp" startOffset="50%" text-anchor="middle"></textPath></text></svg>'
  +'<div class="mapbox"><div class="map"></div><div class="pin"></div></div></div><div class="txt"><p class="t1"></p><p class="t2"></p><p class="t3"></p></div><span class="osm">© OpenStreetMap-Mitwirkende</span></div>';
}
...
function kpRender(c,st,beispiel){
 ...
 const mb=c.el.querySelector(".mapbox");mb.className="mapbox "+(st.design==="klassik"?st.form:"quadrat");mb.style.background=s.bg;
 ...
 if(hz!==null&&hz!==undefined&&hz!=="")e2.textContent=hz;
 ...
 const m1=st.design==="sil"?(c.city||""):[d,c.city].filter(Boolean).join("  ·  "),m2=co;
 c.el.querySelector(".t3").innerHTML=kpEsc(m1)+(m1&&m2?'<span class="sep">  ·  </span>':"")+`<span class="z">${kpEsc(m2)}</span>`;
 koord.innerHTML=(ct?`${kpEsc(kpDms(ct.lat,true))}<br>${kpEsc(kpDms(ct.lng,false))}`:"")+(d?`<br><span class="kd">${kpEsc(d)}</span>`:"");
}
...
window.karteStart=function(pdp){
 ...
 let tabs=null;
 if(n>1){ tabs=document.createElement("div"); tabs.className="kp-tabs"; tabs.innerHTML=Array.from({length:n},(_,i)=>`<button type="button" aria-pressed="${i===0}">Poster ${i+1}</button>`).join(""); buehne.appendChild(tabs); }
 const zeige=(i)=>{ aktiv=i; cards.forEach((c,j)=>{ c.wrap.hidden=j!==i; }); if(tabs) tabs.querySelectorAll("button").forEach((b,j)=>b.setAttribute("aria-pressed",j===i)); const c=cards[i]; if(c){ setTimeout(()=>{ c.map.resize(); kpRender(c,st,c.bsp); },30); } };
 ...
 for(let i=1;i<=n;i++){
  ...
  const bspOrt=n>1?KP_BEISPIEL_SET[(i-1)%KP_BEISPIEL_SET.length]:KP_BEISPIEL;
  const map=new maplibregl.Map({container:wrap.querySelector(".map"),style:kpStyle(st.stil),center:[bspOrt.lon,bspOrt.lat],zoom:vor.z,attributionControl:false,dragRotate:false,pitchWithRotate:false,touchZoomRotate:true,preserveDrawingBuffer:true,fadeDuration:0,
  ...
  map.on("move",()=>{ c.center=map.getCenter(); kpRender(c,st,c.bsp); });
  ...
  ["q","t","n1","n2","d"].forEach(k=>{ const e=felder(i,k); if(!e) return;
   ...
   } else { e.addEventListener("input",()=>{ c.cfg[k]=e.value; kpRender(c,st,c.bsp); schreiben(); }); e.addEventListener("focus",()=>zeige(i-1)); e.addEventListener("blur",()=>{ e.value=kpSauber(e.value); c.cfg[k]=e.value; kpRender(c,st,c.bsp); schreiben(); }); }
  });
  cards.push(c); kpRender(c,st,bsp);
 ...
 function anwenden(c,r){ c.city=r.city; c.dirty=false; c.geo=true; const z=(r.type==="house"||r.type==="street")?Math.max(c.map.getZoom(),15):c.map.getZoom(); c.map.jumpTo({center:[r.lon,r.lat],zoom:z}); c.center=c.map.getCenter(); kpRender(c,st,c.bsp); schreiben(); }
 ...
 ["kpDesign","kpForm","kpStil","kpSchrift","kpMarker"].forEach(id=>{ const e=q("#"+id); if(!e) return; e.addEventListener("input",()=>{ const alt=st.design; lesen();
   ...
   cards.forEach(c=>{ if(id==="kpStil"||id==="kpDesign") kpApplyStyle(c.map,st.stil); kpRender(c,st,c.bsp); });
   setTimeout(()=>cards.forEach(c=>{ c.map.resize(); kpRender(c,st,c.bsp); }),60); setTimeout(()=>cards.forEach(c=>c.map.resize()),500); schreiben(); }); });
 ...
 window.addEventListener("resize",()=>cards.forEach(c=>{ c.map.resize(); kpRender(c,st,c.bsp); }));
 if(document.fonts&&document.fonts.ready) document.fonts.ready.then(()=>cards.forEach(c=>kpRender(c,st,c.bsp)));
...
window.karteDruck=function(cfg,breite,hoehe){
 ...
 kpRender(c,st,null);
 let fonts=false, karte=false; const fertig=()=>{ if(fonts&&karte){ kpRender(c,st,null); window.__karte_fertig=true; } };
 (document.fonts&&document.fonts.ready?document.fonts.ready:Promise.resolve()).then(()=>{ fonts=true; fertig(); });
 map.once("idle",()=>{ karte=true; fertig(); });
 window.__karte=c;
};
})();
...
function kaufbalken(){
  ...
  const auffrischen=()=>{ const p=q("#preis"); const preis=p?p.textContent.trim():"";
    ...
    kbN.textContent=istDig?(preis+" · Digitale Datei, sofort per E-Mail"):(preis+(gr?" · "+gr:"")+" · in 3–5 Werktagen bei dir"); };
...
function initSeite(){
  ...
  const pdp=q(".pdp"); if(pdp){ let groesse=0, farbe="";
    ...
    pdp.querySelectorAll(".thumbs button").forEach(b=>b.addEventListener("click",()=>{pdp.querySelectorAll(".thumbs button").forEach(x=>x.setAttribute("aria-pressed","false")); b.setAttribute("aria-pressed","true");
      if(b.dataset.live){ const lm=q("#liveMain"); if(lm) lm.hidden=false; q("#galMain").hidden=true; return; }   /* Karten-Poster: zurueck zur Live-Vorschau */
      const im=q("#galMain img"); const k=b.dataset.k; im.removeAttribute("src"); im.dataset.k=k; bilderSetzen(); if(!window.IMGMAP) im.src="img/"+k+".jpg"; const lm=q("#liveMain"); if(lm){lm.hidden=true; q("#galMain").hidden=false;}}));
    ...
    (()=>{ const lb=q("#lb"), g=q("#galMain"); if(!lb||!g) return;
      ...
      const reihe=()=>[...pdp.querySelectorAll(".thumbs button")].map(b=>b.dataset.k);
      const srcFuer=(k)=>{ const b=[...pdp.querySelectorAll(".thumbs button")].find(x=>x.dataset.k===k); const t=b&&b.querySelector("img"); if(t&&(t.currentSrc||t.getAttribute("src"))) { const im=q("#galMain img"); if(im&&im.dataset.k===k&&(im.currentSrc||im.src)) return im.currentSrc||im.src; } if(window.IMGMAP&&IMGMAP[k]) return IMGMAP[k]; return "img/"+k+".jpg"; };
      let offen=false;
...
function suchen(){
  ...
  const zeigen=(txt)=>{
    ...
    treffer.forEach(e=>{
      ...
      im.src="img/s/"+e.b+".jpg"; im.onerror=()=>{ im.onerror=null; im.src="img/"+e.b+".jpg"; };
