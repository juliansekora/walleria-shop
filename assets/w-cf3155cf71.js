...
const bildUrl=k=>"img/"+k+".jpg"+((window.BILDV&&BILDV[k])?"?v="+BILDV[k]:""); const esc=s=>String(s==null?"":s).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;");
const RABATT=0.30;
...
function kaufbalken(){
  ...
  const auffrischen=()=>{ const p=q("#preis"); const preis=p?p.textContent.trim():"";
    ...
    kbN.textContent=istDig?(preis+" · Digitale Datei per E-Mail, spätestens in 24 h"):(preis+(gr?" · "+gr:"")+" · in 3–5 Werktagen bei dir"); };
  ...
  fetch(window.WALLERIA_NEWSLETTER,{method:"POST",
   ...
   .then(r=>r.json()).then(a=>{
      ...
      if(e2==="pruefe_postfach"){
        feld.value=""; try{ localStorage.setItem("wal_nl_ok","1"); }catch(x){} const bl=q("#willkBlase"); if(bl) bl.hidden=true;
...
function willkSchliessen(){ const w=q("#willk"); if(!w||w.hidden) return; w.hidden=true; document.body.classList.remove("ck-offen"); try{ localStorage.setItem(WK_ZU,String(Date.now())); }catch(e){} }
function willkNachCookie(){ if(!wkSeite||wkLs(WK_NL)==="1") return; if(Date.now()-(+wkLs(WK_ZU)||0) < WK_TAG) return;
  clearTimeout(wkTimer); wkTimer=setTimeout(willkOeffnen,7000);
  const beiScroll=()=>{ if(scrollY>window.innerHeight*0.6){ window.removeEventListener("scroll",beiScroll); willkOeffnen(); } }; window.addEventListener("scroll",beiScroll,{passive:true}); }
