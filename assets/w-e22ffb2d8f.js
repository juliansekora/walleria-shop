...
function kaufbalken(){
  ...
  /* Handy: Preis auf dem ersten Bildschirm, direkt unter den Brotkrumen - Anzeigen-Besucher sahen sonst
     erst Bild und Titel und sprangen ab (04.09.2026). Auf dem Desktop per CSS ausgeblendet. */
  try{
    const crumbs=q(".crumbs"); const preis=q("#preis");
    if(crumbs&&preis){
      const druck=[...document.querySelectorAll("#groessen button")].map(b=>parseFloat(b.dataset.preis)*(1-RABATT)).filter(x=>isFinite(x));
      const pe=q(".preis"); const dig=parseFloat(String((pe&&pe.dataset.digital)||"").replace(",","."));
      const teile=[];
      if(isFinite(dig)&&dig>0) teile.push("<b>"+eur(dig)+"</b> als Datei");
      if(druck.length) teile.push("gedruckt ab <b>"+eur(Math.min(...druck))+"</b>");
      if(teile.length){ const o=document.createElement("div"); o.className="kb-oben"; o.innerHTML=teile.join(" · ")+' · <a href="#inKorb">Zum Kauf ↓</a>'; crumbs.insertAdjacentElement("afterend",o);
        o.querySelector("a").addEventListener("click",ev=>{ev.preventDefault(); knopf.scrollIntoView({behavior:"smooth",block:"center"});}); }
    }
  }catch(e){}
