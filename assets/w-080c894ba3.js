...
function ladeGoogleAds(){
  ...
  const js=document.createElement("script"); js.async=true; js.src="https://www.googletagmanager.com/gtag/js?id="+encodeURIComponent(id);
...
function googleSeitenEreignis(){
  ...
  if(pdp){
    ...
    gtag("event","view_item",{currency:"EUR",value:isFinite(preis)?preis:undefined,items:[{item_id:pdp.dataset.id||"",item_name:pdp.dataset.titel||""}]});
    ...
    if(knopf && !knopf.dataset.gads){ knopf.dataset.gads="1"; knopf.addEventListener("click",()=>{ if(!window.gtag) return;
      ...
      gtag("event","add_to_cart",{currency:"EUR",value:isFinite(pr)?pr:undefined,items:[{item_id:pdp.dataset.id||"",item_name:pdp.dataset.titel||""}]}); }); }
  }
  if(location.pathname.indexOf("kasse")>-1 && korb.length){
    gtag("event","begin_checkout",{currency:"EUR",value:korbSumme(),items:korb.map(p=>({item_id:p.produkt||p.titel,item_name:p.titel,quantity:p.menge||1,price:Number(p.preis)}))});
  }
  const p=new URLSearchParams(location.search);
