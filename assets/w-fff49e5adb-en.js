...
function kaufbalken(){
  ...
  bar.innerHTML='<span class="kb-preis"><b id="kbPreis"></b><small id="kbGroesse"></small></span><button class="btn" type="button" id="kbKauf">Add to cart</button>';
  ...
  const auffrischen=()=>{ const p=q("#preis"); if(p) kbP.textContent=p.textContent;
    ...
    kbG.textContent=istDig?"Digital file · instantly by email":(akt?(akt.firstChild&&akt.firstChild.textContent?akt.firstChild.textContent.trim()+" cm":""):""); };
  ...
  const pruefen=()=>{ const r=knopf.getBoundingClientRect();
    /* Buy button not in view - whether scrolled past at the top or still below: then display price + button permanently.
...
function bestellStatus(){
  ...
  async function dateienHolen(nummer,mail,versuch){
    ...
    try{
      ...
      if(!fertig.length){
        stand.textContent="Your posters are being drawn right now – this takes one to three minutes. This page will update automatically.";
        if(versuch<20) setTimeout(()=>dateienHolen(nummer,mail,versuch+1),12000); else stand.textContent="This is taking longer than usual. We’ll email you as soon as your files are ready.";
        return;
      }
...
function dankeSeite(){
  ...
  async function holen(nummer,mail,versuch){
    ...
    try{
      ...
      const fertig=d.pakete.filter(x=>x.zustand==="fertig");
      if(!fertig.length){
        stand.textContent="Your posters are being drawn right now – this takes one to three minutes. "
          +"This page will update automatically.";
        if(versuch<20) setTimeout(()=>holen(nummer,mail,versuch+1), 12000);
        else stand.textContent="This is taking longer than usual. We’ll email you as soon as your files are ready.";
        return;
      }
...
function initSeite(){
  ...
  const pdp=q(".pdp"); if(pdp){ let groesse=0, farbe="";
      ...
      usFeld.addEventListener("change",()=>{
        ...
        leser.onload=()=>{
          ...
          bild.onload=()=>{
            ...
            vorschau.innerHTML="";
            const im=document.createElement("img"); im.src=daten; im.alt="Your ultrasound image";
            const weg=document.createElement("button"); weg.type="button"; weg.className="usb-weg";
            weg.textContent="Choose another image";
            ...
            fetch(window.WALLERIA_KUNDENBILD,{method:"POST",
              ...
              .then(r=>r.json()).then(a=>{
                if(a&&a.ergebnis==="ok"){ usKennung=a.kennung; stand.textContent="transferred"; }
    ...
    q("#inKorb").addEventListener("click",()=>{
      ...
      const usFrei=!!usFeld;
      if(!usFrei && pdp.dataset.pers==="1" && !(kn&&kn.value.trim())){ if(kn) feldFehler(kn,"Please first "+((kn.dataset.persFeld)||"the name").replace(/\s*\((freiwillig|optional)\)/i,"")+" eintragen"); zaehl("warenkorb-abgewiesen:name"); return; }
      ...
      if(art==="digital") korb.push({produkt:pid,titel:pdp.dataset.titel,img:korbBild,url,menge:1,groesse:"Digitale Datei (JPG in 5 sizes up to 50×70 cm)",name:kn?kn.value.trim():"",angaben,beschriftung,farbe,preis:dig});
      else korb.push({produkt:pid,titel:pdp.dataset.titel,img:korbBild,url,menge:1,groesse:b.firstChild.textContent+" cm",name:kn?kn.value.trim():"",angaben,beschriftung,farbe,preis:parseFloat(b.dataset.preis)*(1-RABATT)}); korbSpeichern(); korbZeigen(); q("#drawer").classList.add("open"); toast("Added to cart"); }); }
  /* Checkout */
const kf=q("#kForm"); if(kf){
  ...
  async function codePruefen(){
    ...
    try{ const r=await fetch(window.STRIPE_CHECKOUT_URL,{method:"POST",headers:{"Content-Type":"application/json","apikey":window.SUPABASE_KEY},body:JSON.stringify({aktion:"code",code})});
      ...
      if(j.ok){ gutschein={code:j.code,percent:j.percent||0,betrag:j.betrag||0}; stand.hidden=false; stand.className="small ok"; stand.textContent="Voucher "+j.code+" redeemed: "+(j.percent?j.percent+" % Rabatt":eur(j.betrag)+" Rabatt")+"."; }
  ...
  kf.addEventListener("submit",async e=>{ e.preventDefault();
    ...
    if(window.STRIPE_CHECKOUT_URL){
      try{ const r=await fetch(window.STRIPE_CHECKOUT_URL,{method:"POST",headers:{"Content-Type":"application/json","apikey":window.SUPABASE_KEY},body:JSON.stringify({posten,kunde,code:(q("#kCode")&&q("#kCode").value.trim().toUpperCase())||""})});
        ...
        const m=String(err.message||""); q("#kHinweis").textContent=/gutschein/i.test(m)?m:"Payment currently not possible: "+m;
