...
function briefmarkeSVG(name){ const B=NP.B,H=NP.H; const ini=initiale(name)||"M"; let s=`<rect width="${B}" height="${H}" fill="#F4EFE3"/>`;
  ...
  return `<svg viewBox="0 0 ${B} ${H}" xmlns="http://www.w3.org/2000/svg">${s}</svg>`; }
const LIVE_DESIGNS=["rosie","sophie","briefmarke"];
function istLive(d){ d=String(d||"rosie"); return LIVE_DESIGNS.some(x=>d.endsWith(x)); }
function npSVG(design,name){ name=(name||"").trim().slice(0,40).replace(/[<>"]/g,"")||"Mila"; const d=String(design||"rosie"); const svg=d.endsWith("briefmarke")?briefmarkeSVG(name):(d==="sophie"?sophieSVG(name):rosieSVG(name)); return svg.replace(/&(?!amp;)/g,"&amp;"); }
if(document.fonts&&document.fonts.load){ Promise.all([document.fonts.load('40px "Lilita One"'),document.fonts.load('40px "Titan One"'),document.fonts.load('40px "Archivo Black"')]).then(()=>{ document.querySelectorAll(".poster.np").forEach(el=>{ if(el.dataset.name!==undefined && istLive(el.dataset.design)) npRender(el,el.dataset.name); }); }).catch(()=>{}); }
