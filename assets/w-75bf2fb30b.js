...
function kaufbalken(){
  ...
  try{
    const felder=q(".pers-felder"), live=q("#liveMain");
    if(felder && live && matchMedia("(max-width:900px)").matches){
      const hs=document.createElement("h3"); hs.className="pers-titel"; hs.textContent="Personalisieren";
      live.insertAdjacentElement("afterend",felder); felder.insertAdjacentElement("beforebegin",hs); felder.classList.add("bei-vorschau");
      /* Namensfeld ganz nach oben - "Für wie viele Kinder?" kommt danach */
      const kn=q("#kName"); if(kn){ let w=kn; while(w.parentElement && w.parentElement!==felder) w=w.parentElement; if(w.parentElement===felder) felder.insertAdjacentElement("afterbegin",w); }
    }
  }catch(e){}
