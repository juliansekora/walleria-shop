...
function hrBlock(saetze,x0,x1,y0,y1,fam,cap,farbe){ const n=saetze.length; const breite=x1-x0; const pxH=Math.floor((y1-y0)/(n*1.42));
  const px=Math.max(24,Math.min(cap,pxH,Math.min.apply(null,saetze.map(t=>hrPassgenau(fam,t,breite)))));
  const m=hrMetrik(fam,px); const zh=m.a+m.b; const spalt=((y1-y0)-n*zh)/Math.max(1,n-1); let y=y0, s=""; const mitte=(x0+x1)/2;
...
function hrRegelgroesse(saetze,breite,cap,fam){ return Math.min(cap,Math.min.apply(null,saetze.map(t=>hrPassgenau(fam,t,breite)))); }
const HR_DESIGNS={
  "b2-titel":function(p){ const B=HR.B,H=HR.H,I=DEF_FONT.instr; let s=hrText(B/2,590,defGesperrt(p.name),I,44,"#8A8272");
    const y0=740,y1=H-240,bh=246,spalt=((y1-y0)-7*bh)/6,x0=170,x1=B-170; const px=hrRegelgroesse(p.regeln,x1-x0-300,126,I); const m=hrMetrik(I,px); let y=y0;
    for(const t of p.regeln){ const cy=y+bh/2; s+=hrText(x0+230,cy-(m.a+m.b)/2+6,t,I,px,"#FBF6EA","start"); y+=bh+spalt; } return s; },
  "b4-botanik":function(p){ const B=HR.B,H=HR.H,I=DEF_FONT.instr; const x0=290,y0=560,x1=B-290,y1=H-560; let s=hrText(B/2,y0+320,defGesperrt(p.name),I,44,"#A6512F");
    const px=hrRegelgroesse(p.regeln,x1-x0-150,100,I); const m=hrMetrik(I,px); const ry0=y0+460,ry1=y1-120; const spalt=((ry1-ry0)-7*(m.a+m.b))/6; let y=ry0;
    for(const t of p.regeln){ s+=hrText(B/2,y,t,I,px,"#3E5C50"); y+=(m.a+m.b)+spalt; } return s; },
  "b5-sonne":function(p){ const B=HR.B,H=HR.H,I=DEF_FONT.instr; let s=hrText(B/2,850,defGesperrt(p.name),I,44,"#B85C38");
    const y0=1000,y1=H-260,bh=220,spalt=((y1-y0)-7*bh)/6,x0=190,x1=B-190; const px=hrRegelgroesse(p.regeln,x1-x0-130,118,I); const m=hrMetrik(I,px); let y=y0;
    for(const t of p.regeln){ s+=hrText(B/2,y+bh/2-(m.a+m.b)/2+8,t,I,px,"#FFF6E4"); y+=bh+spalt; } return s; },
...
/* Eigene Regeln: eine pro Zeile, hoechstens sieben; weniger -> mit den Standardregeln aufgefuellt (wie die Produktion) */
function hrRegeln(text){ let r=String(text||"").replace(/;/g,"\n").split(/\n/).map(z=>z.trim()).filter(Boolean).slice(0,7);
  if(r.length<7){ for(const st of HR.STD){ if(r.length>=7) break; if(r.indexOf(st)<0) r.push(st); } } return r; }
...
function initSeite(){
  ...
  const pdp=q(".pdp"); if(pdp){ let groesse=0, farbe="";
    ...
    if(hrDesign){ const e=q("#pf1"); if(e) e.addEventListener("input",hrVorschau);
    ...
    const vorschau=()=>{ if(defInfo){ defVorschau(); return; } if(hrDesign){ hrVorschau(); return; } if(!lm) return;
...
function ladeDienste(){
  ...
  if(consentAllows("statistik")) ladeGA4();
